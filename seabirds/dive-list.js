    ...
    target.innerHTML = dives.length
      ? dives
          .map((d) => {
            ...
            return `<button class="dive-row" data-id="${Core.esc(d.id)}"><span class="dive-number-cell"><small>Dive #</small><b>${d.diveNumber ?? "&mdash;"}</b></span><span class="dive-summary"><span class="dive-title-line"><b>${Core.esc(d.site)}</b></span><span class="dive-card-details"><span class="dive-card-date">${date.ymd} RT ${Core.esc(startTime || "--:--")} &ndash; ${Core.esc(endTime || "--:--")}</span><span class="dive-card-divider" aria-hidden="true">|</span><em>${Core.esc(classification)}</em><span>${Core.converted(+d.depth).toFixed(1)} ${state.settings.depth}</span><span>${d.duration} min</span><span class="card-temperature">${d.temp == null ? "&mdash;" : Core.temperature(+d.temp).toFixed(0) + "&deg;" + state.settings.temp.toUpperCase()}</span></span></span><span class="dive-row-arrow">&rsaquo;</span></button>`;
          })
          .join("")
