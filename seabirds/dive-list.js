    ...
    target.innerHTML = dives.length
      ? dives
          .map((d) => {
            ...
            return `<button class="dive-row" data-id="${Core.esc(d.id)}"><span class="dive-number-cell"><small>Dive #</small><b>${d.diveNumber ?? "—"}</b></span><span class="date-tile full-date"><b>${date.ymd}</b><small>Start ${Core.esc(startTime || "—")}</small><small>End ${Core.esc(endTime || "—")}</small></span><span class="site"><span class="dive-title-line"><b>${Core.esc(d.site)}</b></span><span class="dive-card-details"><em>${Core.esc(classification)}</em><span>${Core.converted(+d.depth).toFixed(1)} ${state.settings.depth}</span><span>${d.duration} min</span><span class="card-temperature">${d.temp == null ? "—" : Core.temperature(+d.temp).toFixed(0) + "°" + state.settings.temp.toUpperCase()}</span></span>${subtitle ? `<small>${Core.esc(subtitle)}</small>` : ""}</span><span class="dive-row-arrow">›</span></button>`;
          })
          .join("")
