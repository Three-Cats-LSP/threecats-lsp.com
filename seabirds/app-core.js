...
function navigate(id){if(id==='dashboard')id='dives';document.querySelectorAll('.view').forEach(node=>node.classList.toggle('active',node.id===id));document.querySelectorAll('.nav').forEach(node=>node.classList.toggle('active',node.dataset.view===id));const name=document.getElementById('viewName');if(name)name.textContent=({dives:'Dive log',devices:'Devices',settings:'Settings'})[id]||'Dive log';document.querySelector('.sidebar')?.classList.remove('open')}
